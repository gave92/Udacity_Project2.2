...
exec_visualization = ['show_pcl','show_bev','show_detection_performance'] # options are 'show_range_image', 'show_bev', 'show_pcl', 'show_labels_in_image', 'show_objects_and_labels_in_bev', 'show_objects_in_bev_labels_in_camera', 'show_tracks', 'show_detection_performance', 'make_tracking_movie'
    ...
    try:
        ...
        if (configs_det.use_labels_as_objects==True):
            ...
            if 'detect_objects' in exec_list:
                ...
                if 'perform_tracking' in exec_list:
                    ...
                    arch = configs_det.arch[:configs_det.arch.rfind("_")] if "_" in configs_det.arch else configs_det.arch
                    detections = load_object_from_file(results_fullpath, data_filename, 'detections_' + arch + '_' + str(configs_det.conf_thresh), cnt_frame)
        ...
        if 'measure_detection_performance' in exec_list:
            ...
            if 'perform_tracking' in exec_list:
                ...
                arch = configs_det.arch[:configs_det.arch.rfind("_")] if "_" in configs_det.arch else configs_det.arch
                det_performance = load_object_from_file(results_fullpath, data_filename, 'det_performance_' + arch + '_' + str(configs_det.conf_thresh), cnt_frame)
        ...
        if 'show_range_image' in exec_list:
            ...
            cv2.namedWindow("range_image", cv2.WINDOW_NORMAL) #MOD
