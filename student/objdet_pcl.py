...
def show_pcl(pcl):
    ...
    global first_frame, vis, pcd

    # step 1 : initialize open3d with key callback and create window
    def right_arrow_callback(vis):
        vis.close()
        return False  # Return True if geometry needs to be updated

    if first_frame:
        vis = o3d.visualization.VisualizerWithKeyCallback()
        vis.create_window(window_name='PCL', width=800, height=600)
        vis.register_key_callback(262, right_arrow_callback)
...
def show_range_image(frame, lidar_name):
    ...
    ri_intensity = (ri_intensity-percentile_1)*  255 / (percentile_99 - percentile_1)
    ri_intensity = np.clip(ri_intensity, 0, 255)
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    #intensity_map[np.int_(lidar_pcl_top[:, 0]), np.int_(lidar_pcl_top[:, 1])] = np.amax(lidar_pcl_top[:, 3])*10 * lidar_pcl_top[:, 3] / (np.amax(lidar_pcl_top[:, 3])-np.amin(lidar_pcl_top[:, 3]))
